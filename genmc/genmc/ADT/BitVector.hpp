...
inline uint64_t alignTo(uint64_t Size, uint64_t A)
{
...
template <typename BitVectorT> class const_set_bits_iterator_impl {
	const BitVectorT *Parent{}; /* cannot be const ref: need assignable for range concepts */
	int Current = 0;
...
public:
	...
	const_set_bits_iterator_impl() = default;
	...
	const_set_bits_iterator_impl(const_set_bits_iterator_impl &&) = default;

	auto operator=(const const_set_bits_iterator_impl &)
		-> const_set_bits_iterator_impl & = default;
	auto operator=(const_set_bits_iterator_impl &&) -> const_set_bits_iterator_impl & = default;
	...
	{
		assert(&Parent == &Other.Parent && "Comparing iterators from different BitVectors");
...
class BitVector {
	...
	using Storage = genmc::SmallVector<BitWord>;
...
public:
	...
	const_set_bits_iterator set_bits_begin() const { return const_set_bits_iterator(*this); }
	const_set_bits_iterator set_bits_end() const { return const_set_bits_iterator(*this, -1); }
	auto set_bits() const { return std::ranges::subrange(set_bits_begin(), set_bits_end()); }
	...
	bool any() const
	{
		return std::ranges::any_of(Bits, [](BitWord Bit) { return Bit != 0; });
	}
	...
	int find_last_in(unsigned Begin, unsigned End) const
	{
		...
		for (unsigned i = LastWord + 1; i >= FirstWord + 1; --i) {
			...
			if (Copy != 0)
				return (CurrentWord + 1) * BITWORD_SIZE - std::countl_zero(Copy) -
				       1;
		}
	...
	int find_last_unset_in(unsigned Begin, unsigned End) const
	{
		...
		for (unsigned i = LastWord + 1; i >= FirstWord + 1; --i) {
			...
			if (Copy != ~BitWord(0)) {
				unsigned Result = (CurrentWord + 1) * BITWORD_SIZE -
						  std::countl_one(Copy) - 1;
	...
	int find_next_unset(unsigned Prev) const { return find_first_unset_in(Prev + 1, Size); }
	...
	int find_prev_unset(unsigned PriorTo) const { return find_last_unset_in(0, PriorTo); }
...
private:
	...
	void wordShl(uint32_t Count)
	{
		...
		// here.
		std::copy(Bits.begin(), Bits.begin() + NumWords - Count, Bits.begin() + Count);
	...
	void init_words(bool t) { std::ranges::fill(Bits, 0 - (BitWord)t); }
...
inline BitVector::size_type capacity_in_bytes(const BitVector &X) { return X.getMemorySize(); }
