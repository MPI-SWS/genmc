...
protected:
	...
	void assertSafeToReferenceAfterClear(ItTy From, ItTy To)
	{
		if constexpr (std::is_pointer_v<ItTy> &&
			      std::is_same_v<std::remove_const_t<std::remove_pointer_t<ItTy>>,
	...
	{
		if constexpr (std::is_pointer_v<ItTy> &&
			      std::is_same_v<std::remove_cv_t<std::remove_pointer_t<ItTy>>, T>) {
			if (From == To)
				return;
...
void SmallVectorTemplateBase<T, TriviallyCopyable>::takeAllocationForGrow(T *NewElts,
									  size_t NewCapacity)
{
...
protected:
	...
	static void uninitialized_copy(It1 I, It1 E, It2 Dest)
	{
		if constexpr (std::is_pointer_v<It1> && std::is_pointer_v<It2> &&
			      std::is_same_v<std::remove_const_t<std::remove_pointer_t<It1>>,
...
public:
	...
	explicit SmallVector(size_t Size) : SmallVectorImpl<T>(N) { this->resize(Size); }
	...
	explicit SmallVector(const iterator_range<RangeTy> &R) : SmallVectorImpl<T>(N)
	{
...
extern template class genmc::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class genmc::SmallVectorBase<uint64_t>;
#endif
...
// 	static SmallVector<T, N> getEmptyKey() { return {DenseMapInfo<T>::getEmptyKey()}; }

// 	static SmallVector<T, N> getTombstoneKey() { return {DenseMapInfo<T>::getTombstoneKey()}; }
...
swap(genmc::SmallVectorImpl<T> &LHS, genmc::SmallVectorImpl<T> &RHS)
{
	LHS.swap(RHS);
}
...
swap(genmc::SmallVector<T, N> &LHS, genmc::SmallVector<T, N> &RHS)
{
	LHS.swap(RHS);
}
