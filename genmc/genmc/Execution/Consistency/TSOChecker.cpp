...
bool TSOChecker::visitCalc71Iterative(std::vector<DFSWorklistEntry> &worklist, View &calcRes) const
{
...
View TSOChecker::visitCalc71(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
	View calcRes;
...
auto TSOChecker::checkCalc71(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
...
bool TSOChecker::visitCalc72Iterative(std::vector<DFSWorklistEntry> &worklist, View &calcRes) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
View TSOChecker::visitCalc72(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
	View calcRes;
...
auto TSOChecker::checkCalc72(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
...
bool TSOChecker::visitCalc73Iterative(std::vector<DFSWorklistEntry> &worklist, View &calcRes) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
View TSOChecker::visitCalc73(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
	View calcRes;
...
auto TSOChecker::checkCalc73(const EventLabel *lab) const
{
	[[maybe_unused]] auto &g = *lab->getParent();
...
void TSOChecker::calculateSaved([[maybe_unused]] EventLabel *lab) {}
...
bool TSOChecker::visitCoherenceIterative(std::vector<DFSWorklistEntry> &worklist,
...
bool TSOChecker::visitConsAcyclic1Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
		...
		case 1: {
			...
			if (auto pLab = g.po_imm_succ(lab); pLab)
				if (true && genmc::isa<ThreadCreateLabel>(pLab)) {
					auto status = visitedConsAcyclic1_3.getStatus(
						pLab->getStamp().get());
					if (status == NodeStatus::unseen) {
						worklist.emplace_back(3, pLab);
			...
			if (auto pLab = g.po_imm_succ(lab); pLab)
				if (true && genmc::isa<ThreadFinishLabel>(pLab)) {
					auto status = visitedConsAcyclic1_3.getStatus(
						pLab->getStamp().get());
					if (status == NodeStatus::unseen) {
						worklist.emplace_back(3, pLab);
		...
		case 2: {
			...
			if (auto pLab = g.po_imm_succ(lab); pLab)
				if (true && pLab->isSC() && genmc::isa<ReadLabel>(pLab)) {
					auto status = visitedConsAcyclic1_3.getStatus(
						pLab->getStamp().get());
					if (status == NodeStatus::unseen) {
						worklist.emplace_back(3, pLab);
		...
		case 3: {
			...
			if (status == NodeStatus::unseen) {
				++visitedConsAcyclic1Accepting;
				visitedConsAcyclic1_3.setIncr(lab->getStamp().get(),
							      visitedConsAcyclic1Accepting,
							      NodeStatus::entered);
			...
			if (true && genmc::isa<ThreadJoinLabel>(lab))
				if (auto pLab = g.po_imm_succ(lab); pLab) {
					auto status = visitedConsAcyclic1_0.getStatus(
						pLab->getStamp().get());
					if (status == NodeStatus::unseen) {
			...
			if (true && genmc::isa<ThreadStartLabel>(lab))
				if (auto pLab = g.po_imm_succ(lab); pLab) {
					auto status = visitedConsAcyclic1_0.getStatus(
						pLab->getStamp().get());
					if (status == NodeStatus::unseen) {
			...
			if (true && lab->isSC() && genmc::isa<WriteLabel>(lab))
				if (auto pLab = g.po_imm_succ(lab); pLab) {
...
bool TSOChecker::visitError2([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError2Iterative(std::vector<DFSWorklistEntry> &worklist,
					       const View &v) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
...
bool TSOChecker::visitError3([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError3Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitRHSUnlessError3Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitError4([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError4Iterative(std::vector<DFSWorklistEntry> &worklist,
					       const View &v) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
...
bool TSOChecker::visitError5([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError5Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitRHSUnlessError5Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitError6([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError6Iterative(std::vector<DFSWorklistEntry> &worklist,
					       const View &v) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
...
bool TSOChecker::visitError7([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError7Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitRHSUnlessError7Iterative(std::vector<DFSWorklistEntry> &worklist) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
		worklist.pop_back();
...
bool TSOChecker::visitError8([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessError8Iterative(std::vector<DFSWorklistEntry> &worklist,
					       const View &v) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
...
bool TSOChecker::visitWarning9([[maybe_unused]] const EventLabel *lab) const { return false; }

bool TSOChecker::visitLHSUnlessWarning9Iterative(std::vector<DFSWorklistEntry> &worklist,
						 const View &v) const
{
	while (!worklist.empty()) {
		auto [stateId, lab, isFinishing] = worklist.back();
...
TSOChecker::checkErrors([[maybe_unused]] const EventLabel *lab,
...
bool TSOChecker::isConsistent([[maybe_unused]] const EventLabel *lab) const
{

	return true && checkConsAcyclic1(lab);
}

bool TSOChecker::isConsistent([[maybe_unused]] const ExecutionGraph &g) const
{
...
void TSOChecker::recomputeCacheCounters([[maybe_unused]] const ExecutionGraph &g) const {}

void TSOChecker::resetCacheCounters() const {}

void TSOChecker::maybeDecreaseCacheCounters([[maybe_unused]] const EventLabel *lab) const {}

void TSOChecker::maybeIncreaseCacheCounters([[maybe_unused]] const EventLabel *lab) const {}
