...
SAddr getStaticAllocBegin(const VSet<std::pair<SAddr, SAddr>> &allocMap, SAddr addr)
{
	...
	auto it = std::lower_bound(
		...
		[](const decltype(p) &itV, const decltype(p) &v) { return itV.second < v.first; });
	VERIFY(it != allocMap.end() && addr >= it->first && addr <= it->second);
	return it->first;
}
...
std::string Interpreter::getStaticName(SAddr addr) const
{
	...
	VERIFY(staticNames.count(sBeg));
	auto gv = staticNames.at(sBeg);
	auto gvID = MI->idInfo.VID[gv];
	VERIFY(MI->varInfo.globalInfo.count(gvID));
...
void *Interpreter::getStaticAddr(SAddr addr) const
{
	...
	VERIFY(isStaticallyAllocated(addr));
	auto sBeg = getStaticAllocBegin(staticAllocas, addr);
	VERIFY(staticValueMap.count(sBeg));
...
EventLabel::EventLabelKind getUnlockKind(Instruction &I)
{
	...
	auto flags = dyn_cast<ConstantInt>(op->getValue())->getZExtValue();
	VERIFY(GENMC_KIND(flags) == GENMC_KIND_PLOCK);
...
std::pair<EventLabel::EventLabelKind, EventLabel::EventLabelKind> getCasKinds(AtomicCmpXchgInst &I)
{
	...
	VERIFY(GENMC_KIND(flags) == GENMC_KIND_CONFIRM);
...
void Interpreter::visitAllocaInst(AllocaInst &I)
{
	...
	auto ret = CALL_DRIVER(handleMalloc, currDbgInfo(), currPos(), MemToAlloc,
...
#define IMPLEMENT_READ_VISIT(__kind)                                                               \
	case EventLabel::__kind: {                                                                 \
		if (I.getOrdering() == AtomicOrdering::NotAtomic)                                  \
			val = CALL_DRIVER(handleNALoad, currDbgInfo(ptr), currPos(), ptr, size,    \
					  GET_DEPS(deps));                                         \
		else                                                                               \
			val = CALL_DRIVER(handle##__kind, currDbgInfo(ptr), currPos(),             \
					  toGenMCOrdering(I.getOrdering()), ptr, size, nullptr,    \
					  getCurrentAnnotConcretized(), GET_DEPS(deps));           \
		break;                                                                             \
	}

	GenMCDriver::ResultType<SVal> val;
...
void Interpreter::visitStoreInst(StoreInst &I)
{
	...
	if (ord == MemOrdering::NotAtomic)
		CALL_DRIVER(handleNAStore, currDbgInfo(ptr), currPos(), ptr, asize,
			    GV_TO_SVAL(val, typ), GET_DEPS(deps));
	else
		CALL_DRIVER(handleWrite, currDbgInfo(ptr), currPos(), ord, ptr, asize,
...
#define IMPLEMENT_CAS_VISIT(__kindR, __kindW)                                                      \
	case switchPair(EventLabel::__kindR, EventLabel::__kindW): {                               \
		...
		auto sDeps = makeEventDeps(getDataDeps(getCurThr().id, I.getPointerOperand()),     \
					   ...
					   nullptr);                                               \
		CALL_DRIVER(handle##__kindW, currDbgInfo(ptr), currPos(),                          \
	...
	case switchPair(EventLabel::HelpingCas, EventLabel::HelpingCas): {
		CALL_DRIVER(handleHelpingCas, currDbgInfo(ptr), currPos(),
			    toGenMCOrdering(I.getSuccessOrdering()), ptr, size,
			    GV_TO_SVAL(cmpVal, typ), GV_TO_SVAL(newVal, typ), GET_DEPS(lDeps));
		break;
	}
	default:
		UNREACHABLE();
	...
	result.AggregateVal.push_back(SVAL_TO_GV(res, typ));
...
#define IMPLEMENT_FAI_VISIT(__kindR, __kindW)                                                      \
	case switchPair(EventLabel::__kindR, EventLabel::__kindW): {                               \
		auto ret = CALL_DRIVER(handle##__kindR, currDbgInfo(ptr), currPos(),               \
				       toGenMCOrdering(I.getOrdering()), ptr, size,                \
				       toGenMCBinOp(I.getOperation()), val, getWriteAttr(I),       \
				       nullptr, std::nullopt, GET_DEPS(deps));                     \
                                                                                                   \
		auto retVal = std::get_if<SVal>(&ret);                                             \
		if (!retVal)                                                                       \
			return;                                                                    \
		...
		updateDataDeps(getCurThr().id, &I, currPos());                                     \
		updateAddrPoDeps(getCurThr().id, I.getPointerOperand());                           \
		auto newVal = executeRMWBinOp(res, val, size, toGenMCBinOp(I.getOperation()));     \
		CALL_DRIVER(handle##__kindW, currDbgInfo(ptr), currPos(),                          \
			    toGenMCOrdering(I.getOrdering()), ptr, size, newVal, getWriteAttr(I),  \
			    GET_DEPS(deps));                                                       \
		break;                                                                             \
	}
	...
	SetValue(&I, SVAL_TO_GV(res, typ), ECStack().back());
}
...
#define IMPLEMENT_LOCK_VISIT(__kindR, __kindW)                                                     \
	case switchPair(EventLabel::__kindR, EventLabel::__kindW): {                               \
		auto ret = CALL_DRIVER(handle##__kindR, currDbgInfo(addr), currPos(), addr, size,  \
		...
		if (!retVal)                                                                       \
			return;                                                                    \
                                                                                                   \
		if (*retVal == SVal(0))                                                            \
			CALL_DRIVER(handle##__kindW, currDbgInfo(addr), currPos(), addr, size,     \
				    GET_DEPS(deps));                                               \
		else                                                                               \
...
{
	CALL_DRIVER(handleUnlockWrite, currDbgInfo(addr), currPos(), MemOrdering::Release, addr,
		    size, SVal(0), WriteAttr::None, GET_DEPS(deps));
}
...
void Interpreter::callOptBegin(Function *F, const std::vector<GenericValue> &ArgVals,
			       const std::unique_ptr<EventDeps> &specialDeps)
{
	auto ret = CALL_DRIVER(handleOptional, currDbgInfo(), currPos());
	auto expand = std::get_if<bool>(&ret);
	VERIFY(expand);
				 ...
				 const std::unique_ptr<EventDeps> &specialDeps)
{
	if (ArgVals[0].IntVal.getBoolValue()) {
		auto ret = CALL_DRIVER(handleThreadKill, currDbgInfo(), currPos());
		VERIFY(std::holds_alternative<std::monostate>(ret));
		ECStack().clear();
	}
}
...
{
	if (!ArgVals[0].IntVal.getBoolValue()) {
		auto ret = CALL_DRIVER(handleAssume, currDbgInfo(), currPos(),
			     ...
			     const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto deps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id),
				  getAddrPoDeps(getCurThr().id), nullptr);
	auto ret = CALL_DRIVER(handleMalloc, currDbgInfo(), currPos(), size,
			       alignof(std::max_align_t), StorageDuration::SD_Heap,
			       StorageType::ST_Volatile, AddressSpace::AS_User, nullptr, {},
			       GET_DEPS(deps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
				    ...
				    const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto deps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id),
				  getAddrPoDeps(getCurThr().id), nullptr);
	auto ret = CALL_DRIVER(handleMalloc, currDbgInfo(), currPos(), size, align,
			       StorageDuration::SD_Heap, StorageType::ST_Volatile,
			       AddressSpace::AS_User, nullptr, {}, GET_DEPS(deps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
	returnValueToCaller(F->getReturnType(), SVAL_TO_GV(*result, F->getReturnType()));
			      ...
			      const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto ret = CALL_DRIVER(handleMalloc, currDbgInfo(), currPos(), size,
			       alignof(std::max_align_t), StorageDuration::SD_Heap,
			       StorageType::ST_Durable, AddressSpace::AS_User, nullptr, {},
			       GET_DEPS(deps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
				 ...
				 const std::unique_ptr<EventDeps> &specialDeps)
{
	auto deps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id),
				  getAddrPoDeps(getCurThr().id), nullptr);
	auto ret = CALL_DRIVER(handleThreadJoin, currDbgInfo(), currPos(),
			       ArgVals[0].IntVal.getLimitedValue(), GET_DEPS(deps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
	returnValueToCaller(F->getReturnType(), SVAL_TO_GV(*result, F->getReturnType()));
				...
				const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleNAStore, currDbgInfo(lock), currPos(), lock, size, SVal(0),
				   ...
				   const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto ret = CALL_DRIVER(handleTrylockCasRead, currDbgInfo(ptr), currPos(), ptr, size,
			       getCurrentAnnotConcretized(), GET_DEPS(specialDeps));
	auto res = std::get_if<SVal>(&ret);
	if (!res)
		return;

	auto cmpRes = *res == SVal(0);
	if (cmpRes)
		CALL_DRIVER(handleTrylockCasWrite, currDbgInfo(ptr), currPos(), ptr, size,
			    GET_DEPS(specialDeps));
				   ...
				   const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleNAStore, currDbgInfo(lock), currPos(), lock, size, SVal(-1),
		    GET_DEPS(specialDeps));
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleNAStore, currDbgInfo(cvar), currPos(), cvar, size, SVal(0),
		    GET_DEPS(specialDeps));
	...
	auto annot = std::move(Annotation(
		...
		Annotation::ExprVP(
			SgtExpr<AnnotID>::create(
				...
				.release())));
	auto ret = CALL_DRIVER(handleCondVarWaitRead, currDbgInfo(cvar), currPos(),
			       MemOrdering::Relaxed, cvar, size, nullptr, std::move(annot),
			       GET_DEPS(specialDeps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
				    ...
				    const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleCondVarSignalWrite, currDbgInfo(cvar), currPos(), MemOrdering::Relaxed,
		    cvar, size, SVal(1), WriteAttr::None, GET_DEPS(specialDeps));
				   ...
				   const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleCondVarBcastWrite, currDbgInfo(cvar), currPos(), MemOrdering::Relaxed,
		    cvar, size, SVal(1), WriteAttr::None, GET_DEPS(specialDeps));
				     ...
				     const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleCondVarDestroyWrite, currDbgInfo(cvar), currPos(), MemOrdering::Relaxed,
		    cvar, size, SVal(-1), WriteAttr::None, GET_DEPS(specialDeps));
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	auto deps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id),
				  getAddrPoDeps(getCurThr().id), nullptr);
	auto ret = CALL_DRIVER(handleMalloc, currDbgInfo(), currPos(),
			       getTypeSize(F->getReturnType()), alignof(std::max_align_t),
			       StorageDuration::SD_Heap, StorageType::ST_Volatile,
			       AddressSpace::AS_Internal, nullptr, {}, GET_DEPS(deps));
	auto result = std::get_if<SVal>(&ret);
	if (!result)
		return;
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleWrite, currDbgInfo(hp), currPos(), MemOrdering::Release, hp, asize,
		    SVal(), WriteAttr::None, GET_DEPS(specialDeps));
