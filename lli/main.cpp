...
static llvm::cl::opt<ExplorationMode> clExplorationMode(
	"mode", llvm::cl::cat(clGeneral), llvm::cl::init(ExplorationMode::verify),
	llvm::cl::desc("Choose exploration mode:"),
	llvm::cl::values(clEnumValN(ExplorationMode::verify, "verify",
				    "Exhaustive exploration (default)"),
			 clEnumValN(ExplorationMode::random, "random", "Randomized exploration"),
			 clEnumValN(ExplorationMode::estimate, "estimate",
				    "State-space size estimation only")));
...
static llvm::cl::opt<unsigned long long>
	clArbitraryScheduleSeed("schedule-seed", llvm::cl::value_desc("N"),
				llvm::cl::cat(clDebugging),
				     ...
				     const VerificationResult &res)
{
	...
	if (res.boundExceeding) {
		VERIFY(conf->boundType != BoundType::round);
...
static void printRandomResults(const std::shared_ptr<const Config> &conf,
			       const VerificationResult &res)
{
	PRINT(VerbosityLevel::Error, "{}", res.message);
	PRINT(VerbosityLevel::Error, "{}.\n",
	      (!res.status.has_value() ? "*** Random exploration complete.\nNo errors were detected"
				       : "\n*** Random exploration unsuccessful"));
	...
	GENMC_DEBUG(if (conf->countMootExecs) {
		PRINT(VerbosityLevel::Error, " (+ {} mooted)", res.exploredMoot);
	};);
}
...
auto sample(const LLIConfig &lliConfig, std::shared_ptr<const Config> conf,
...
auto main(int argc, char **argv) -> int
{
	...
	case ExplorationMode::estimate: {
		res = estimate(lliConfig, conf, moduleUP, modInfo);
		printEstimationResults(conf, begin, res);
		break;
	}
	case ExplorationMode::random: {
		res = sample(lliConfig, conf, std::move(moduleUP), std::move(modInfo));
		printRandomResults(conf, res);
		break;
	}
	case ExplorationMode::verify:
		...
		if (conf->estimate) {
			LOG(VerbosityLevel::Tip,
			    "Estimating state-space size before verification. For better "
			    "performance, you can use --disable-estimation.");
			res = estimate(lliConfig, conf, moduleUP, modInfo);
			printEstimationResults(conf, begin, res);
