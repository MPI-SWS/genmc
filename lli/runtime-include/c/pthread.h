...
#define PTHREAD_COND_INITIALIZER __VERIFIER_COND_INITIALIZER
			 ...
			 unsigned int __count)
{
	...
	assert(__count > 0 && "pthread_barrier_init() called with an invalid value!");
	assert(__atomic_load_n(&__barrier->__count, __ATOMIC_RELAXED) == 0 &&
	       "pthread_barrier_init() called on an already initialized barrier!");

	__atomic_store_n(&__barrier->__count, __count, __ATOMIC_RELAXED);
	__atomic_store_n(&__barrier->__private, 0, __ATOMIC_RELAXED);
...
{
	int iVal = __atomic_load_n(&__barrier->__count, __ATOMIC_RELAXED);
...
int pthread_barrier_destroy(pthread_barrier_t *__barrier)
{
	...
	__VERIFIER_annotate_begin(GENMC_KIND_BARRIER);
	__atomic_store_n(&__barrier->__count, 0, __ATOMIC_RELAXED);
	__VERIFIER_annotate_end(GENMC_KIND_BARRIER);
