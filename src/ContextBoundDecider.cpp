...
{
	const auto *nLab = g.getNextLabel(Event(t, s.getMax(t)));
	return nLab && (llvm::isa<ThreadJoinLabel>(nLab) ||
			llvm::isa<LockCasReadLabel>(nLab) ||
...
auto isEnabled(const ExecutionGraph &g, const View &v, int t) -> bool
{
	...
	const auto *nLab = g.getNextLabel(last);
	if (!nLab)
		return false;
	...
	if (auto *lLab = llvm::dyn_cast<LockCasReadLabel>(nLab)) {
		...
		return sLab && std::none_of(sLab->readers_begin(), sLab->readers_end(),
...
void shrinkViewByOneInPlace(View &v, int t)
{
	v.setMax(Event(t, v.getMax(t)).prev());
}
...
auto ContextBoundDecider::getSlack() const -> unsigned
{
	...
	auto unstableThreads = std::count_if(g.begin(), g.end(), [](const auto &t) {
			return std::ranges::any_of(t, [](const auto &lab) {
						return !lab->isStable();
					});
			});
...
auto ContextBoundDecider::calculate(View v, int t) const -> unsigned
{
	...
	for (auto tp = 0u; tp < g.getNumThreads(); tp++) {
		...
		if (!v.getMax(tp) || !isSCMaximal(g, v, tp))
			continue;
		auto sp = shrinkViewByOne(v, tp);
		auto bound = calculate(sp, tp) +
			((t != tp) && isEnabled(g, v, tp) ? 1 : 0);
