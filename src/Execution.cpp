...
#define IMPLEMENT_READ_VISIT(__kind)                                                               \
	case EventLabel::EventLabelKind::EL_##__kind: {                                            \
		val = CALL_DRIVER_RESET_IF_NONE(                                                   \
			handleLoad, __kind##Label::create(currPos(), I.getOrdering(), ptr, size,   \
							  atyp, GET_DEPS(deps)));                  \
		break;                                                                             \
	}
...
#define IMPLEMENT_CAS_VISIT(nameR, nameW)                                                          \
	case switchPair(EventLabel::EventLabelKind::EL_##nameR,                                    \
			EventLabel::EventLabelKind::EL_##nameW): {                                 \
		ret = CALL_DRIVER_RESET_IF_NONE(                                                   \
			handleLoad,                                                                \
			nameR##Label::create(currPos(), I.getSuccessOrdering(), ptr, size, atyp,   \
					     GV_TO_SVAL(cmpVal, typ), GV_TO_SVAL(newVal, typ),     \
					     getWriteAttr(I), GET_DEPS(lDeps)));                   \
		if (!ret.has_value())                                                              \
			return;                                                                    \
		...
		if (cmpRes) {                                                                      \
			...
			CALL_DRIVER(handleStore,                                                   \
				    nameW##Label::create(currPos(), I.getSuccessOrdering(), ptr,   \
							 size, atyp, GV_TO_SVAL(newVal, typ),      \
	...
	case switchPair(EventLabel::EL_HelpingCas, EventLabel::EL_HelpingCas):
		if (!CALL_DRIVER_RESET_IF_FALSE(
			    handleHelpingCas,
			    HelpingCasLabel::create(currPos(), I.getSuccessOrdering(), ptr, size,
...
SVal Interpreter::executeAtomicRMWOperation(SVal oldVal, SVal val, ASize size,
					    AtomicRMWInst::BinOp op)
{
	switch (op) {
	case AtomicRMWInst::Xchg:
		WARN_ON_ONCE(
			getDepTracker(), "unsupported-xchg-deps",
...
void Interpreter::visitAtomicRMWInst(AtomicRMWInst &I)
{
	...
	if (thr.tls.count(ptr)) {
		GenericValue oldVal = thr.tls[ptr];
		auto newVal = executeAtomicRMWOperation(GV_TO_SVAL(oldVal, typ), val, size,
							I.getOperation());
...
#define IMPLEMENT_FAI_VISIT(nameR, nameW)                                                          \
	case switchPair(EventLabel::EventLabelKind::EL_##nameR,                                    \
			EventLabel::EventLabelKind::EL_##nameW): {                                 \
		ret = CALL_DRIVER_RESET_IF_NONE(                                                   \
			handleLoad, nameR##Label::create(currPos(), I.getOrdering(), ptr, size,    \
							 atyp, I.getOperation(), val,              \
...
void Interpreter::handleLock(SAddr addr, ASize size, const EventDeps *deps)
{
	...
	if (!getCurThr().isBlocked()) {
		if (ret == SVal(0))
			CALL_DRIVER(handleStore, LockCasWriteLabel::create(currPos(), addr, size,
									   GET_DEPS(deps)));
		else
				...
				const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleStore, WriteLabel::create(currPos(), AtomicOrdering::NotAtomic, lock,
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleStore,
		    BInitWriteLabel::create(currPos(), AtomicOrdering::NotAtomic, barrier, size,
					    atyp, value, GET_DEPS(specialDeps)));
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto oldVal = CALL_DRIVER_RESET_IF_NONE(
		handleLoad, BIncFaiReadLabel::create(
				    currPos(), AtomicOrdering::AcquireRelease, barrier, asize, atyp,
				    AtomicRMWInst::BinOp::Sub, SVal(1), GET_DEPS(specialDeps)));
	...
	CALL_DRIVER(handleStore,
		    BIncFaiWriteLabel::create(currPos(), AtomicOrdering::AcquireRelease, barrier,
					      asize, atyp, newVal, GET_DEPS(specialDeps)));

	CALL_DRIVER(handleLoad, BWaitReadLabel::create(currPos(), AtomicOrdering::Acquire, barrier,
				     ...
				     const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleStore,
		    BDestroyWriteLabel::create(currPos(), AtomicOrdering::NotAtomic, barrier, size,
				  ...
				  const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleStore, WriteLabel::create(currPos(), AtomicOrdering::Release, hp, asize,
						    atyp, SVal()));
	return;
}
...
static const std::unordered_map<std::string, SmpFenceType> smpFenceTypes = {
	{"mb", SmpFenceType::MB},     {"rmb", SmpFenceType::RMB}, {"wmb", SmpFenceType::WMB},
	{"ba", SmpFenceType::MBBA},   {"aa", SmpFenceType::MBAA}, {"as", SmpFenceType::MBAS},
				   ...
				   const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	CALL_DRIVER(handleFence,
		    SmpFenceLabelLKMM::create(currPos(), llvm::AtomicOrdering::Monotonic, ft,
					      GET_DEPS(deps)));
	return;
}
...
void Interpreter::callRCUReadLockLKMM(Function *F, const std::vector<GenericValue> &ArgVals,
				      const std::unique_ptr<EventDeps> &specialDeps)
{
	CALL_DRIVER(handleRCULockLKMM, RCULockLabelLKMM::create(currPos()));
	return;
}
			      ...
			      const std::unique_ptr<EventDeps> &specialDeps)
{
	auto deps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id),
				  getAddrPoDeps(getCurThr().id), nullptr);
	CALL_DRIVER(handleCLFlush,
		    CLFlushLabel::create(currPos(), GVTOP(ArgVals[0]), GET_DEPS(deps)));
	return;
}
...
void Interpreter::setInodeTransStatus(void *inode, Type *intTyp, SVal val)
{
	...
	CALL_DRIVER(handleDskWrite,
		    DskJnlWriteLabel::create(currPos(), inodeItrans, getTypeSize(intTyp),
					     TYPE_TO_ATYPE(intTyp), val, GET_JOURNAL_MAPPING(inode),
					     inode));
	return;
}
...
SVal Interpreter::readInodeSizeFS(void *inode, Type *intTyp, const std::unique_ptr<EventDeps> &deps)
{
	...
	return CALL_DRIVER(handleLoad, ReadLabel::create(currPos(), AtomicOrdering::Acquire,
							 inodeIsize, asize, atyp, GET_DEPS(deps)))
		.value();
}
...
{
	auto inodeIsize = GET_INODE_ISIZE_ADDR(inode);
	CALL_DRIVER(handleStore, WriteLabel::create(currPos(), AtomicOrdering::Release, inodeIsize,
						    getTypeSize(intTyp), TYPE_TO_ATYPE(intTyp),
						    newSize, GET_DEPS(deps)));
	return;
}

void Interpreter::updateInodeDisksizeFS(void *inode, Type *intTyp, SVal newSize, SVal ordDataBegin,
					SVal ordDataEnd)
{
	auto inodeIdisksize = GET_INODE_IDISKSIZE_ADDR(inode);
	...
	for (auto i = 0u; i < count; i++) {
		...
		auto val = CALL_DRIVER(handleLoad,
				       ReadLabel::create(currPos(), AtomicOrdering::NotAtomic,
							 loadAddr, size, atyp, GET_DEPS(deps)))
				   .value();

		auto writeAddr = (char *)inodeData + inodeOffset + i;
		CALL_DRIVER(handleDskWrite, DskWriteLabel::create(currPos(), writeAddr, size, atyp,
								  val, GET_DATA_MAPPING(inode)));
	}
	return;
}
				   ...
				   int count, Type *dataTyp, const std::unique_ptr<EventDeps> &deps)
{
	...
	for (auto i = 0u; i < count; i++) {
		...
		CALL_DRIVER(handleStore,
			    WriteLabel::create(currPos(), AtomicOrdering::NotAtomic, storeAddr,
					       asize, atyp, val, GET_DEPS(deps)));
	}
	return;
}
...
void Interpreter::updateDirNameInode(const std::string &name, Type *intTyp, SVal inode)
{
	...
	CALL_DRIVER(handleDskWrite,
		    DskDirWriteLabel::create(currPos(), inodeAddr,
					     getTypeSize(intTyp->getPointerTo()), AType::Pointer,
					     inode, GET_DATA_MAPPING(dirInode)));
...
SVal Interpreter::executeInodeLookupFS(const std::string &filename, Type *intTyp)
{
	...
	return CALL_DRIVER(handleDskRead, DskReadLabel::create(currPos(), inodeAddr,
							       getTypeSize(intTyp->getPointerTo()),
							       AType::Pointer));
}
				       ...
				       const std::unique_ptr<EventDeps> &deps)
{
	...
	auto *info = getVarNameInfo(nullptr, StorageDuration::SD_Heap, AddressSpace::AS_Internal,
				    "inode");
	auto *inode =
		(void *)CALL_DRIVER(
			handleMalloc,
			MallocLabel::create(currPos(), inodeSize, alignof(std::max_align_t),
					    ...
					    std::string("__inode_") + filename, GET_DEPS(deps)))
			.get();
	...
	auto asize = getTypeSize(intTyp);
	auto atyp = TYPE_TO_ATYPE(intTyp);
	CALL_DRIVER(handleStore, WriteLabel::create(currPos(), AtomicOrdering::Release, inodeLock,
				...
				const std::unique_ptr<EventDeps> &deps)
{
	...
	/* Get a fresh fd */
	auto fd = CALL_DRIVER(handleDskOpen,
			      DskOpenLabel::create(currPos(), filename, getTypeSize(intTyp)));
	...
	CALL_DRIVER(handleStore, WriteLabel::create(currPos(), llvm::AtomicOrdering::NotAtomic,
						    fileInode, getTypeSize(intPtrType),
						    AType::Pointer, inode, GET_DEPS(deps)));
	CALL_DRIVER(handleStore,
		    WriteLabel::create(currPos(), llvm::AtomicOrdering::NotAtomic, fileFlags,
				       getTypeSize(intTyp), atyp, flags, GET_DEPS(deps)));
	CALL_DRIVER(handleStore,
		    WriteLabel::create(currPos(), llvm::AtomicOrdering::NotAtomic, fileOffset,
				       getTypeSize(intTyp), atyp, SVal(0), GET_DEPS(deps)));
	CALL_DRIVER(handleStore,
		    WriteLabel::create(currPos(), llvm::AtomicOrdering::Release, filePosLock,
				       getTypeSize(intTyp), atyp, SVal(0), GET_DEPS(deps)));
	CALL_DRIVER(handleStore,
		    WriteLabel::create(currPos(), llvm::AtomicOrdering::Release, fileCount,
				       getTypeSize(intTyp), atyp, SVal(1), GET_DEPS(deps)));
				    ...
				    const std::unique_ptr<EventDeps> &deps)
{
	...
	if (length.getSigned() > oldIsize.getSigned()) {
		...
		zeroDskRangeFS((void *)inode.get(), oldIsize, length,
			       Type::getInt8Ty(intTyp->getContext()));
...
SVal Interpreter::executeCloseFS(SVal fd, Type *intTyp, const std::unique_ptr<EventDeps> &deps)
{
	...
	auto ret = CALL_DRIVER(handleLoad,
			       FaiReadLabel::create(currPos(), AtomicOrdering::AcquireRelease,
						    fileCount, asize, atyp, AtomicRMWInst::Sub,
						    SVal(1), GET_DEPS(deps)))
			   .value();

	auto newVal = executeAtomicRMWOperation(ret, SVal(1), asize, AtomicRMWInst::Sub);

	CALL_DRIVER(handleStore,
		    FaiWriteLabel::create(currPos(), AtomicOrdering::AcquireRelease, fileCount,
					  asize, atyp, newVal, GET_DEPS(deps)));
...
SVal Interpreter::executeRenameFS(const std::string &oldpath, SVal oldInode,
				  const std::string &newpath, SVal newInode, Type *intTyp)
{
			       ...
			       const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	result = SVAL_TO_GV(executeRenameFS(oldpath, source, newpath, target, intTyp),
			    F->getReturnType());
...
bool Interpreter::shouldUpdateInodeDisksizeFS(void *inode, Type *intTyp, SVal iSize, SVal wOffset,
					      SVal countVal, SVal &dSize)
{
				...
				SVal count, const std::unique_ptr<EventDeps> &deps)
{
	...
	auto flags =
		CALL_DRIVER(handleLoad, ReadLabel::create(currPos(), AtomicOrdering::NotAtomic,
							  flagsOffset, asize, atyp, GET_DEPS(deps)))
			.value();
	...
	auto fileInode = GET_FILE_INODE_ADDR(file);
	auto *inode =
		(void *)CALL_DRIVER(handleLoad,
				    ReadLabel::create(currPos(), llvm::AtomicOrdering::Monotonic,
						      fileInode, getTypeSize(intPtrType),
						      AType::Pointer, GET_DEPS(deps)))
			.value()
			.get();
			     ...
			     const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto offset = CALL_DRIVER(handleLoad,
				  ReadLabel::create(currPos(), llvm::AtomicOrdering::NotAtomic,
						    fileOffset, asize, atyp, GET_DEPS(deps)))
			      .value();
...
SVal Interpreter::executeWriteChecksFS(void *inode, Type *intTyp, SVal flags, SVal offset,
				       SVal count, SVal &wOffset,
				       const std::unique_ptr<EventDeps> &deps)
{
	...
	return SVal(
		std::min(count.getSigned(), (int64_t)MI->fsInfo.maxFileSize - wOffset.getSigned()));
}
					 ...
					 const std::unique_ptr<EventDeps> &deps)
{
	...
	auto iSize = readInodeSizeFS(inode, intTyp, deps);
	if (MI->fsInfo.delalloc &&
	    shouldUpdateInodeDisksizeFS(inode, intTyp, iSize, wOffset, count, dSize)) {
	...
	do {
		...
		auto bytes = std::min<unsigned long>(MI->fsInfo.blockSize,
						     dataCount); /* should be small enough */
...
SVal Interpreter::executeWriteFS(void *file, Type *intTyp, void *buf, Type *bufElemTyp, SVal offset,
				 SVal count, const std::unique_ptr<EventDeps> &deps)
{
	Type *intPtrType = intTyp->getPointerTo();
	auto asize = getTypeSize(intTyp);
...
void Interpreter::executeFsyncFS(void *inode, Type *intTyp)
{
	...
	/* do the fsync() */
	CALL_DRIVER(handleDskFsync,
		    DskFsyncLabel::create(currPos(), inode, getTypeSize(MI->fsInfo.inodeTyp)));
	return;
}
			      ...
			      const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	auto fileInode = GET_FILE_INODE_ADDR(file);
	auto *inode = (void *)CALL_DRIVER(
			      handleLoad,
			      ReadLabel::create(currPos(), llvm::AtomicOrdering::Monotonic,
						fileInode, getTypeSize(retTyp->getPointerTo()),
						AType::Pointer, GET_DEPS(deps)))
			      .value()
			      .get();
				 ...
				 const std::unique_ptr<EventDeps> &deps)
{
	...
	case GENMC_SEEK_CUR: {
		...
		CALL_DRIVER(handleStore,
			    WriteLabel::create(currPos(), llvm::AtomicOrdering::NotAtomic,
					       fileOffset, asize, atyp, newOffset.get(),
					       GET_DEPS(deps)));
		return newOffset;
	}
	default:
...
{
	return isFsInvalidRecCode(fCode) || (fCode == InternalFunctions::FN_OpenFS &&
